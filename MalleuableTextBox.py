#credits
#https://code.i-harness.com/en/q/2c9737 - havent used it yet
#http://code.activestate.com/recipes/578886-automatic-resized-text-widget-tkinter/
try:
...
class AutoResizedText(Frame):
	...
	def _on_keypress(self, event):
		...
		# We check whether it is a punctuation or normal key
			...
			old_text = self._textarea.get("1.0", END)				#where in text will insertion be, what is being inserted
	...
	def insert(self,word):
		...
		for x in range(0,len(word)):
			# old_text=self._insert_character_into_message(old_text, self._textarea.index(INSERT),word[x])
			self._textarea.insert(INSERT, word[x])
			# print(self.__textarea)
